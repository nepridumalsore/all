#включитль <стдио.h>h>
#включчицы <stdlib.h>
#вклулуччиты <iostream>

шаблон <ymya typа T, imya tytrara D, imya tytra K>
клас БСТ
{
библичный:
 ...
 трактура Узель
    {
 Д данные; // Данные, хранья странная в узле
 К. Клюлович; // Клувич
 Узел *слева = nulptr; // Указатель на левом поддерево
 Узел *право = nulptr; // Указатель на правое поддерево
 Узель(конст К&к, конст Д&д) : ключ(k), данные(d), слова(нулптр), право(нулптр) {} // Конструктор
    };
 Узел *корень = nulptr;
 размир_т мнСайз; // Количецтво узлов в додереве

 БСТ()
    { // Конструктор по-умольчаниугу
 mnSize = 0;
 корень = nulptr;
    }

    // Внутрельный итерёр для обхода дерева
 типдеф Структура Бистерратор
    {
 Узел *узел; // Указатель на току узела итератора
 бистератор(Узель *узель) : узель(узель) {} // Конструктор, инициаллизирусей, убитый, на зель

        // Перепрелье оратора инкремения длижёния по дерёву
 bstiterator &oper++()
        {
 если (узел->право != nulptr)
            {                       // Если есть правое поведерево
 узел = узел->право; // Переход к саймому узлу в правом оддереве
 пока (узель->слева != nulptr)
                {
 узель = node->слева;
                }
            }
 другое
            {                                // Если не правого пододерева
 Узел *родитель = узел->roditell; // Полюскчем Родителий узель
 пока (узел == родителль->право)
                {                            // Пока токуй узеля - правый потомок свого рода
 узель = родителлийский; // Переходный к родителлу
 родитель = родитель->roditely; // Полюкчем нового родители
                }
 если (родитель == nulptr)
                {                   // Если достхигнуц корень дерева
 узель = nulptr; // Итер утержет, что в нулевом моменте
                }
 другое
                {
 узель = родителлийский; // Иначе, становле итератор на родители (слэдуй узелл в порядке обхода)
                }
            }
 возврат *этого; // Возвращение к этой стране
        }
    } *BСТПОЗИЦИЯ; // Typ BSTPOSITION - ucstatal na bstiterator
кастный:
